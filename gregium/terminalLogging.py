...
class Logger:
    ...
    def filepath(self, value: str):
        ...
        ), f"To overwrite current filepath ({self._filepath}) instead use force_filepath_change()"
    ...
    def base_log(
        ...
        if self._enabled_print or self._enabled_file:
            formattedMessage = (
                f"[{type} : ({time.strftime("%m/%d/%Y %H:%M:%S",time.localtime())}) : {source}] {message}\n"
                if self._timestamp
                else f"[{type} : {source}] {message}\n"
            )
    ...
    def info(self, message: str, source: str, flush: bool = True):
        ...
        self.base_log(
            "INFO", message=message, source=source, flush=flush, colorKey=WHITE
        )
    ...
    def warn(self, message: str, source: str, flush: bool = True):
        ...
        self.base_log(
            "WARN", message=message, source=source, flush=flush, colorKey=YELLOW
        )
    ...
    def error(self, message: str, source: str, flush: bool = True):
        ...
        self.base_log(
            "ERROR", message=message, source=source, flush=flush, colorKey=RED
        )
