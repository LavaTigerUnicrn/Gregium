            ...
            if compMode == "var":
                if ltr == "=":
                    ...
                    if not lSpace and foundSpace > 0 and not hasWarned:
                        hasWarned = True
                        LOGGER.warn(
                            f"Many spaces found in variable of line {n+1}, variable names will have spaces removed"
                        )
...
def load(fileName: str = None, loadAllEnv: bool = False, ignoreCWD: bool = False):
    ...
    # If a file path is not given, try to find a .grg in the current working directory
        ...
        if len(posEnv) == 0:
            raise ValueError(
                ".grg file not found, add (fileName) parameter to continue"
            )
        ...
        # and the user does not specify that they want multiple files loaded
            ...
            raise ValueError(
                "multiple .grg files found, add (fileName) parameter to continue or set (loadAllEnv) to True"
            )
...
def save(fileName: str = None):
    ...
    # If no file is given, look for .grg files in the current working directory
        ...
        # Raise en error if many .grg files are present
            ...
            raise ValueError(
                "multiple .grg files found, add (fileName) parameter to continue"
            )
